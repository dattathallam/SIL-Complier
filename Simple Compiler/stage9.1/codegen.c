...
int codegen(struct Tnode* root)
{
	...
	if(root -> NODE == NODE_EXPR)
	{
		Ttemp = root;
		Argtemp = Arg_callee;
		while( Ttemp -> VALUE != 1 && Ttemp  != NULL )
		{
			if(Ttemp -> Ptr1 -> NODE == NODE_ID)
			{
				Argtemp = Arg_callee;
				for (i = 1 ; i < Ttemp -> Ptr1 -> VALUE; i++)
				{
					Argtemp = Argtemp -> NEXT;
				}
				if(Argtemp -> TYPE == SYMBOL_AMP_INTEGER || Argtemp -> TYPE == SYMBOL_AMP_BOOLEAN )
					isamp = 1;
			}
			a = codegen(Ttemp -> Ptr1);
			printf("IN HERE 1 : %s \n",Ttemp -> Ptr1 -> NAME );
			// if(Ttemp -> Ptr1 -> NAME != NULL)
			// {
			// 	fprintf(intermediate, "IN HERE : %s\n",Ttemp -> Ptr1 -> NAME);
			// }
			// else
			// {
			// 	fprintf(intermediate, "IN HERE : %d\n",Ttemp -> Ptr1 -> NODE);
			// }
	...
	// BRING THIGS FROM STACK
									if(Argtemp != NULL)
									{
										...
										fprintf(intermediate, "SUB R%d,R%d\n",r2,r3);
										freereg();
										if(isamp && (Argtemp -> TYPE == SYMBOL_AMP_INTEGER ||Argtemp -> TYPE == SYMBOL_AMP_BOOLEAN))
										{
											fprintf(intermediate, "MOV R%d,[R%d]\n",r1,r2); //BP-2-(offset+1)
											isamp = 0;
										}
